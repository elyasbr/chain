...
import {
  PaginateWithdrawCryptoRMapper
} from '../modules/withdraw/rmapper/withdraw-crypto/paginate-withdraw-crypto-r.mapper';
...
import { ChangeWithdrawCryptoDto } from '../modules/withdraw/dtos/withdraw-crypto/change-withdraw-crypto.dto';

@Controller({
  path : "blockchain/withdraw-crypto" ,
  version : "1"
})
@ApiTags("Withdraw Crypto")
@ApiValidationRequest()
@ApiForbiddenRequest()
export class WithdrawCryptoController {
  constructor(private readonly withdrawCryptoService: WithdrawCryptoService) {}

  @Post("/")
  @ApiOperation({ summary: 'Create Withdraw Crypto'  })
  @ApiCreatedObjectResponse(  PaginateWithdrawCryptoRMapper)
  createWithdrawCrypto(@Body() createWithdrawCryptoDto : CreateWithdrawCryptoDto) {
    return this.withdrawCryptoService.createWithdrawCrypto(createWithdrawCryptoDto)
  }


  @Delete("/:withdrawCryptoId")
  @ApiOperation({ summary: 'Delete SpeedWithdraw'  })
  @ApiCreatedArrayResponse(  DeleteResponseDto)
  deleteWithdrawCrypto( @Param('withdrawCryptoId')  withdrawCryptoId : string ) {
    return this.withdrawCryptoService.deleteWithdrawCrypto(withdrawCryptoId )
  }

  @Post("/pagination/:speedWithdrawId")
  @ApiOperation({ summary: 'Pagination Withdraw Crypto'  })
  @ApiCreatedArrayResponse( PaginateWithdrawCryptoRMapper)
  getPagination(@Param('speedWithdrawId') speedWithdrawId : string ,@Body() filterWithdrawCryptoDto : FilterWithdrawCryptoDto) {
    return this.withdrawCryptoService.getPagination(speedWithdrawId , filterWithdrawCryptoDto)
  }
  @Patch("/status/:withdrawCryptoId")
  @ApiOperation({ summary: 'change Status Withdraw Crypto'  })
  @ApiOkObjectResponse(  PaginateWithdrawCryptoRMapper)
  changeStatusWithdrawCrypto(@Param('withdrawCryptoId') withdrawCryptoId: string ,@Body() changeWithdrawCryptoDto : ChangeWithdrawCryptoDto) {
    return this.withdrawCryptoService.changeStatusWithdrawCrypto(withdrawCryptoId ,changeWithdrawCryptoDto)
  }
